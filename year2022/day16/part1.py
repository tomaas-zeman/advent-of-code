...
def run(data: list[str], is_test: bool):
    ...
    solutions = []
    best_flows: dict[int, int] = {}

    paths = PriorityQueue[tuple[int, Valve, int, int, int]]()
    paths.put((0, valves["AA"], 0, 30, 0))
    ...
    while not paths.empty():
        ...
        for next_valve in unvisited_valves:
            ...
            if new_visited_valves_bitmap == all_valves_visited:
                solutions.append(new_flow)
                break

            if new_visited_valves_bitmap not in best_flows:
                best_flows[new_visited_valves_bitmap] = new_flow
            elif best_flows[new_visited_valves_bitmap] > new_flow:
                continue                

            if new_time_left >= 0:
                paths.put((-new_flow, next_valve, new_visited_valves_bitmap, new_time_left, new_flow))
            else:
                solutions.append(flow)
