...
def move_head(point: Point, direction: str):
    ...
    return Point.simple(point.row + row_change, point.column + column_change)
...
def move_tail(head: Point, tail: Point):
    ...
    return Point.simple(row, column)


def count_visited_nodes(data: list[str], number_of_knots: int):
    visited = set()
    knots = [Point.simple(0, 0) for i in range(number_of_knots)]
