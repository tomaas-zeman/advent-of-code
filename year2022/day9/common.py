...
def move_head(point: Point, direction: str):
    ...
    return Point(point.row + row_change, point.column + column_change, int(point.value))
...
def move_tail(head: Point, tail: Point):
    ...
    return Point(row, column, int(tail.value))


def count_visited_nodes(data: list[str], number_of_knots: int):
    visited = set()
    knots = [Point(0, 0, i) for i in range(number_of_knots)]
